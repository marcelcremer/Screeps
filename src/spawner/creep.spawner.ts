...
export class CreepSpawner {
  ...
  spawn() {
    if (this.harvesters.length < this.amount) this.spawnCreep(General.HOME_BASE, Profiles.SMALL_WORKER, 'harvester');
    else if (this.upgrader.length < this.amount + 1) this.spawnCreep(General.HOME_BASE, Profiles.SMALL_WORKER, 'upgrader');
    else if (this.builder.length < this.amount) this.spawnCreep(General.HOME_BASE, Profiles.SMALL_WORKER, 'builder');
  }

  visualizeSpawn() {
    if (Game.spawns[General.HOME_BASE].spawning) {
      Game.spawns[General.HOME_BASE].room.visual.text(
        '🛠️' + (Game.creeps[((Game.spawns[General.HOME_BASE || 'foo'] || {}).spawning || <any>{}).name] || {}).memory['role'],
